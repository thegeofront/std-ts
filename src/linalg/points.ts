...
export function readXYZ(text: string) : XYZ {
...
export function xyzToMultiVector3(xyz: XYZ) : MultiVector3 {
    ...
    if (xindex == -1 && yindex == -1 && zindex == -1) throw new Error("No x, y or z found in .xyz file");

    for (let row = 0; row < height; row++) {
        let slice = buffer.slice(row * width, (row+1) * width);
        if (xindex != -1) newBuffer[row * 3 + 0] = slice[xindex];
        if (yindex != -1) newBuffer[row * 3 + 1] = slice[yindex];
        if (zindex != -1) newBuffer[row * 3 + 2] = slice[zindex];
    }
